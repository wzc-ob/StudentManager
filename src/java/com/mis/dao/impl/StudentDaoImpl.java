...
public class StudentDaoImpl implements IStudentDao{
    ...
    public List<Student> getAllStudent(Pagination pagination) {
        ...
        try {
            ...
            sql = "SELECT * FROM(SELECT ROWNUM NO,s.* FROM (SELECT * FROM student ORDER BY sno ASC) s WHERE ROWNUM<=?) WHERE NO >=?";
            psmt = conn.prepareStatement(sql);
            psmt.setInt(1, end);
            psmt.setInt(2, start);
            rs = psmt.executeQuery();
    ...
    public Student findStudente(String sno){
        ...
        try{            
            ...
            String sql = "select student.sno,student.sname,student.ssex,student.sage,student.sdept,student.photo_url from student inner join userd on student.sno = userd.userid where sno=?";
            psmt = conn.prepareStatement(sql);    
             psmt.setString(1, sno);
            rs = psmt.executeQuery();
            if (rs.next()) {               
